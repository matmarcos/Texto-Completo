...
\title{Lie Algebra Extensions}
\maketitle

\section{Definitions}

In this section we define Lie algebra extensions e some cohomology groups.

\begin{df} Let $K$,$H$ and $L$ be Lie algebras. $L$ is an extension of $K$ by $H$ if there is a exact sequence of Lie algebras,
$$0 \to H \stackrel{ i}{\to} L \stackrel{ s}{\to} K \to 0.$$
\end{df} 

\begin{itemize}
\item if there is an ideal $S$ of $L$ such that $L=S \oplus Ker(s)$ then the extension is \textbf{trivial};
\item if there is a subalgebra $S$ of $L$ such that $L=S \oplus Ker(s)$ then the extension is \textbf{split};
\item if $ker(s)$ is contained in the center of $L$, denoted by $Z(L)$, then $L$ is a \textbf{central} extension.
\end{itemize}

\begin{df}{\label{acao}}Let $K$ and $I$ be algebras. We say that $K$ act on $I$ if there is a algebra morphism $\psi:K \to Der(I)$. In this case, the action will be denoted by $$[a,k]:=\psi(k)(a), k \in K,a \in I.$$
\end{df}

When brackets represents the adjoint representation we also use $\ad{K}{}:K \to Der(I)$ to $\ad{K}{k}(a)=[a,k],$ for all $k \in K$ and $a \in I$. When the domain of representation is clear we just use $\ad{}{k}(a)=[a,k]$.
...
\item The first cohomology group of $K$ and $I$ is definied by $$Z^1(K,I)=\{\nu \in Hom(K,I) \mid \nu([k,h]_K)=[\nu(k),h]-[\nu(h),k], \mbox{ for all } k,h \in K\}.$$
\end{itemize}
\end{df}
